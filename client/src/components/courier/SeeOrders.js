      ...
      if (newStatus === "InProgress") {
        ...
        if (window.confirm("Order Completed?")) {
          ...
          const orderDetails = await axios.get(
            `https://se2024-j6qz.onrender.com/order/${orderId}`
          );
          ...
          if (courierEmail !== orderCourierEmail) {
            setErrorMessage(
              "Unauthorized access: You are not authorized to update this order."
            );
            return;
          }
          updateOrderStatus(orderId, newStatus);
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      ...
      const response = await axios.put(
        `https://se2024-j6qz.onrender.com/order/update`,
        {
          orderId: orderId,
          newStatus: newStatus,
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(
          "https://se2024-j6qz.onrender.com/courierorder"
        );
        if (response.status === 200) {
              ...
              (orderItem, index) =>
                ...
                orderItem.status !== "Completed" && (
                  <tr key={index}>
                    ...
                    <td style={{ textAlign: "center", padding: "15px" }}>
                      {orderItem.vendor_addr}
                    </td>
                    <td style={{ textAlign: "center", padding: "15px" }}>
                      {orderItem.client}
                    </td>
                    <td style={{ textAlign: "center", padding: "15px" }}>
                      {orderItem.client_addr}
                    </td>
                    <td style={{ textAlign: "center", padding: "15px" }}>
                      <button
                        style={{
