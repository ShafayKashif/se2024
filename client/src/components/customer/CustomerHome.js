...
const CustomerHome = () => {
  ...
  const handlePlaceRecentOrder = async (event) => {
    const response = await axios.post(
      "https://se2024-cwdv.onrender.com/CustomerReOrder",
  ...
  const handleSearchSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(
        "https://se2024-cwdv.onrender.com/query",
        {
          type: "food-search",
          query: searchQuery,
        }
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const response = await axios.post(
          "https://se2024-cwdv.onrender.com/CustomerTopVendors"
        );
    ...
    const fetchLastOrder = async () => {
      try {
        const response = await axios.post(
          "https://se2024-cwdv.onrender.com/CustomerLastOrder",
          { clientEmail: sessionStorage.getItem("email") }
        );
    ...
    const fetchCalAndAmount = async () => {
      try {
        const response = await axios.post(
          "https://se2024-cwdv.onrender.com/CustomerCalAndAmount",
          { clientEmail: sessionStorage.getItem("email") }
        );
