...
const CustomerViewCart = () => {
  ...
  const handleSelfPickup = async (event) => {
    ...
    const response = await axios.get(
      "https://se2024-ghn9.onrender.com/CustomerCart"
    );
    ...
    if (response.status === 200) {
      ...
      const items = cart.filter(
        (item) => item.customer_email === customer_email
      );
    ...
    for (let i = 0; i < numItems; i++) {
      ...
      try {
        const response = await axios.get(
          "https://se2024-ghn9.onrender.com/vendors"
        );
        if (response.status === 200) {
          ...
          if (!vendor) {
            alert(
              'Vendor doesnt exist, please reconfirm from "view menu tab".'
            );
            return;
          } else {
      ...
      try {
        const response = await axios.get(
          "https://se2024-ghn9.onrender.com/customers"
        );
        if (response.status === 200) {
      ...
      try {
        const response = await axios.get(
          "https://se2024-ghn9.onrender.com/items"
        );
        if (response.status === 200) {
      ...
      try {
        ...
        const response = await axios.post(
          "https://se2024-ghn9.onrender.com/selfpickupCart",
          {
            vendor_email,
            vendorname,
            customer_email,
      ...
      try {
        const response = await axios.post(
          "https://se2024-ghn9.onrender.com/UpdateQuantity",
          {
            itemId,
  ...
  const handledelivery = async (event) => {
    ...
    for (let i = 0; i < numItems; i++) {
      ...
      try {
        //   console.log("price is: ", price)
        const response = await axios.post(
          "https://se2024-ghn9.onrender.com/customerDeliveryCart",
          {
            vendor_email,
            vendorname,
            customer_email,
  ...
  const fetchItems = async () => {
    try {
      ...
      const response = await axios.post(
        "https://se2024-ghn9.onrender.com/CustomerViewCart",
  ...
  const handleRemoveItem = async (itemId) => {
    try {
      ...
      await axios.post("https://se2024-ghn9.onrender.com/UpdateQuantity", {
        itemId,
      ...
      await axios.post("https://se2024-ghn9.onrender.com/remove-from-cart", {
        itemId,
  ...
  const updateCartItems = async (updatedItems) => {
    try {
      // let customer_email = localStorage.getItem('CustomerEmail');
      const response = await axios.post(
        "https://se2024-ghn9.onrender.com/updateCartItems",
        { customer_email, items: updatedItems }
      );
    } catch (error) {
      ...
      <div className="items-container1">
        {items.map((item) => (
          <div key={item.itemId} className="item-card1">
            ...
            <div>
              <button onClick={() => handleIncreaseQuantity(item.itemId)}>
                +
              </button>
              <button onClick={() => handleDecreaseQuantity(item.itemId)}>
                -
              </button>
